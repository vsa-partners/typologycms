...
class Application_model extends Model {
	...
	var $reset_vars			= array('_req_type','_req_limit','_req_relations','_req_where_fields','_req_return_first','_req_query_fields');
	...
	public function getById($id, $select_set=FALSE) {
			...
			if ($id < 1 ) {
				...
				return (($this->_req_return_first) ? $return : array($return));
			} else if (is_array($id)) {
			...
			$return = (($this->_req_return_first) ? ((count($return) > 0) ? $return[0] : array()) : $return);
...
//pr($fields, 'fields');
//pr($this->_req_query_fields, '_req_query_fields');
	...
	public function search($field, $string, $params=array()) {
...
//			$fields = (is_array($this->search_fields)) ? explode(',', $this->search_fields) : $this->search_fields;
			...
			$this->db->where('MATCH ('.$field.') AGAINST ("'.$string.'" IN BOOLEAN MODE)', NULL, FALSE);
...
//pr($this->db_fields, '_getMatchingFields: db_fields');
	...
	*/
	...
	protected function _addQuerySort($sort_field=NULL, $sort_dir=NULL) {

			if (empty($this->sort_field)) return;

			if (empty($sort_field)) $sort_field = $this->sort_field;
			if (empty($sort_dir))	$sort_dir = $this->sort_dir;
	...
	protected function _addAutoJoin() {
			...
			if (in_array($this->table.'.'.$this->auto_join['table'].'_id', $this->_req_select_fields)) {
				...
				$this->_req_select_fields[] .= ((!empty($this->auto_join['fields'])) ? $this->auto_join['fields'] : $this->auto_join['table'] . '.*');
