...
class Page_redirect_model extends Application_model {

	var $table 		        = 'page_redirect';
	var $id_field	        = 'redirect_id';
	var $db_fields 			= array('redirect_id','old_path','new_path','create_date','update_date','editor_id','type','notes');

	var $title_field		= 'old_path';
	var $sort_field			= 'old_path';
	...
	public function getByPath($path) {
			return $this->get(array('old_path'=>$this->cleanPath($path)));
	}
	...
	public function insertUnique($fields) {

			if ($fields['old_path'] == $fields['new_path']) return false;

			// Clean up any old redirects with the same path
			$old = $this->getByPath($fields['old_path']);
			if (count($old)) {
				$this->delete($old[0][$this->id_field]);
			}
