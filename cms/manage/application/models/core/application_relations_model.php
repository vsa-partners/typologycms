...
class Application_relations_model {
	...
	function get(&$items, $relations=true) {	
			...
			if (is_string($relations)) $relations = array($relations);
			...
			foreach ($this->relations as $relation_key => $relation_params) {
				...
				if (is_array($relations) && !in_array($relation_key, $relations)) continue;
	...
	function getJoinTable($related_model, $relation=array()) {
			...
			foreach ($this->relations as $key => $val) {
				...
				switch ($val['type']) {
                    ...
                    case 'has_many':    
                        ...
                        pr('Error: "has_many" relations have not been build yet.');
						break;
