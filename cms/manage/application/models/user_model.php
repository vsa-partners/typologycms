...
class User_model extends Application_model {
	...
	var $db_fields 		= array('user_id','user','email','type','password','password_options_json','permission_group','permissions','enabled','options','create_date','update_date','login_date');
	...
	public function update($fields=array()) {
			...
			if (!empty($fields['password_new']) && !empty($fields['password_confirm'])) {
				...
				$fields['password'] 				= encryptString($fields['password_new'],$fields['user'],$fields['email']);
	...
	public function changePassword($fields=null) {
			...
			if (encryptString($fields['password_current']) != $this->authentication->get('password')) {
			...
			if (!empty($password_options['previous']) && in_array(encryptString($fields['password_new']), $password_options['previous'])) {
	...
	public function resetPassword($fields=null, $user=null) {
			...
			$update_fields 	= array(
				...
				, 'password'				=> encryptString($new_pass,$user[0][$this->id_field])
