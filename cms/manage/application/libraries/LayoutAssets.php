...
class LayoutAssets {
	...
	public function __construct($config=array()) {
			...
			// TODO: This should not be in site conf
...
pr($path, 'No Exist!');
	...
	public function getTags($type=null) {
			...
			foreach ($this->_assets[$type] as $group_name => $group_files) {
				...
				foreach ($files as $file) {	
					$output .= NL.TAB . sprintf($this->{'_'.$type.'_tag'}, $file, $group_name);
				}
	...
	public function _getPaths($group_name='', $group_files=array(), $type=null) {
			...
			if (!$this->config['assets']['compression_enabled']) return $group_files;
