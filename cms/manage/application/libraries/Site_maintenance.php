...
class Site_maintenance {
	...
	public function deletePageVersions() {
	
			$page_conf = CI()->loadConfig('page');
			
			if (!empty($page_conf['versions']['delete_old']) && ($page_conf['versions']['delete_old'] == TRUE)) {		
				echo '!!! NOT DONE !!!';			
			}
    ...
    public function clearAssetCache($type='all', $zone=null) {
  
            $directory     		= DOCROOT . zonepath('cache/', $zone);
