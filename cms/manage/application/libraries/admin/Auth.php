...
class Auth {
	...
	function __construct($conf=null) {
			...
			$this->CONF = CI()->loadConfig('authentication');
	
			if (!count($this->CONF)) 
				show_error(__METHOD__ . ' Could not initiate library, no config supplied.');
	...
	public function requireModulePermission($request=null) { 
			$this->requirePermission(CI()->module .'_'. $request);
	}
	...
	public function hasPermission($request=null) { 
			...
			if (is_array($request)) {
				...
				if (!$this->_hasPermission($request)) return FALSE;
