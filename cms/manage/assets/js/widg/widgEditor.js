...
**    License Information:
**    -------------------------------------------------------------------------
**    Copyright (c) 2009 Cameron Adams
**
...
var widgStylesheet = "cms/manage/assets/js/widg/widgContent.css";
...
widgSelectBlockOptions.push("<span class='text_red'>", "Red");
/*
...
/* Options on block format select element. Consists of string pairs (option value, option label) */
var widgSelectColorOptions = new Array();
widgSelectColorOptions.push("", "Change color");
widgSelectColorOptions.push("ff00ff", "Pink");
	...
	if (typeof(document.designMode) == "string" && (document.all || document.designMode == "off")){
		...
		for (var i = 0; i < theTextareas.length; i++){
			...
			if (theTextarea.className.classExists("widgEditor")){
				...
				setTimeout("new widgEditor('" + theTextarea.id + "')", 50 * (i));
			}
		}
	}else{
		return false;
...
function widgEditor(replacedTextareaID){
	...
	this.theIframe.frameBorder = "none";
...
widgEditor.prototype.modifyFormSubmit = function(){
	...
	if (typeof theForm.onsubmit != "function"){
		theForm.onsubmit = function() {
			// Remove this temp input. It screws up PHP.
			
			if ((extraId = $(self.theExtraInput.id))) extraId.remove();
...
widgEditor.prototype.writeDocument = function(documentContent)
{
	...
	{
		documentTemplate = documentTemplate.replace(/INSERT:STYLESHEET:END/, '<link rel="stylesheet" type="text/css" href="' + SITEPATH + widgStylesheet + '"></link>');
	}
	...
	if (typeof document.all == "undefined")
	{
		...
		stylesheet.setAttribute("href", SITEPATH + widgStylesheet);
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i]) {
			...
			case "color":
				this.addButton(this.theList.id + "ButtonBold", "widgButtonBold", "Color!", "color");
...
function widgToolbarAction()
{
	...
	switch (this.action)
	{
		...
		case "color":
			...
			theIframe.contentWindow.document.execCommand('forecolor', false, '#ff00ff');			
			break;
